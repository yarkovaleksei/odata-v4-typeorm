...
| Матричные на реальной СУБД | `src/test/matrix/` | Итоговые данные: какие строки вернул запрос |

Матричные идут на той базе, которую выбирает `TEST_DB`: SQLite по умолчанию, PostgreSQL
и MySQL — в отдельных джобах CI. Один и тот же файл теста, три СУБД.

Моки не ловят дефекты генерации SQL — именно поэтому первые три года жизни проекта
все тесты были зелёными при неверно работающих `$filter` и `$expand`. Новые тесты
на поведение пишите матричными. Одного SQLite при этом мало: он регистронезависим
к идентификаторам и прощает то, на чём PostgreSQL падает, — так был найден дефект
с унарным минусом, где алиас уезжал в запрос неподставленным.
...
describe('мой сценарий', () => {
  it('фильтрует по имени', async () => {
    // Без $count=true ответ — обычный массив: отсутствующий $count по OData v4
    // означает false. Объект `{ items, count }` вернётся только на явный запрос счётчика.
    const result = await executeQuery(
      dataSource.getRepository(User),
      { $filter: "name eq 'Alice'" },
      { alias: 'User' }   // ← префикс колонок; совпадать с именем сущности не обязан
    );
...
> Библиотека импортируется в демо **относительным путём** (`../../../src/lib`), а не по имени
> пакета. Раньше стоял импорт по имени вместе с отображением `paths` в
> `examples/server/tsconfig.json`, но на выполнение `paths` не влияет: это правило проверки
> типов, а `ts-node` разрешает модули обычным способом Node. Node же умеет сослаться на пакет
> по его собственному имени через `exports` — и демо грузило `build/`. Слежение за `src/lib`
> при этом работало, сервер послушно перезапускался и показывал прошлую сборку; тайпчек
> проходил, и расхождение ничем себя не выдавало. Загрузку пакета под настоящим именем
> проверяет `yarn build:check`, причём строже — в обоих форматах и на настоящем Node.
...
Коллекция Postman на 88 запросов, включая папку «Ошибки» с ожидаемыми `400`:
