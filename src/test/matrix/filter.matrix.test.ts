...
describe('$filter — арифметика', () => {
  ...
  const cases: readonly MatrixCase[] = [
    ...
    /**
     * Отрицательный литерал, а НЕ унарный минус: `-1` парсер сворачивает в значение `-1`
     * и кладёт в параметр, узла отрицания при этом не возникает. Случай нужен сам по себе —
     * знак внутри литерала легко потерять, — но оператор им не проверяется, для него
     * два случая ниже.
     */
    { name: 'отрицательный литерал', query: { $filter: 'age gt -1 and age lt 30' }, expected: [4] },
    /**
     * Унарный минус как оператор: слева от сравнения стоит `-A.age`, а не параметр.
     * Возрасты 36, 45, 41, 29 дают -36, -45, -41, -29 — меньше -40 ровно двое.
     */
    { name: 'унарный минус перед колонкой', query: { $filter: '-age lt -40' }, expected: [2, 3] },
    /**
     * Отрицание целого выражения, а не одной колонки: скобки обязаны сохраниться,
     * иначе `-(age add 4)` превратилось бы в `-age + 4` и вернуло бы другое множество.
     */
    {
      name: 'унарный минус перед скобками',
      query: { $filter: '-(age add 4) lt -45' },
      expected: [2],
    },
  ];
