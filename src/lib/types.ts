 ...
 *
 * `alias` — префикс колонок в генерируемом SQL, и только он: метаданные сущности
 * `executeQueryByQueryBuilder` берёт у самого построителя, поэтому совпадать с именем класса
 * или таблицы алиасу не нужно (дефект A-03). Совпадение требуется лишь на запасном пути —
 * когда у построителя своих метаданных нет и они ищутся через `connection.getMetadata(alias)`.
 */
