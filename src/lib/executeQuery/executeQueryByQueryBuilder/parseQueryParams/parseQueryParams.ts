 ...
 *
 * ЧЕГО ЗДЕСЬ НЕТ И ГДЕ ЭТО ЖИВЁТ. Модуль только приводит типы; проверка значений идёт дальше
 * по конвейеру, в `executeQueryByQueryBuilder`:
 * - отрицательные `$top` и `$skip` отвергаются там же (`resolveTop`, `assertSkip`)
 *   как `ODataInvalidQueryError`;
 * - верхнюю границу страницы задаёт опция `maxTop`: запрос сверх неё усекается, а не отвергается.
 *
 * Одно поведение так и осталось молчаливым, и это осознанно: `$top=3.14` становится `3`
 * (поведение `parseInt`). Дробный размер страницы — заведомая опечатка клиента, и отвергать
 * запрос ради неё дороже, чем округлить.
 */
