...
describe('TypeOrmVisitor', () => {
  ...
  describe('Логические операторы и арифметика', () => {
    ...
    /**
     * Скобки после унарного минуса проверяются отдельным тестом, потому что нужны они
     * не для приоритета, а для TypeORM: имена колонок он подставляет только там, где
     * перед алиасом стоит пробел, `=`, `(` или начало строки. Без скобок `-u.age`
     * уезжал в SQL неизменённым и падал на PostgreSQL —
     * `invalid reference to FROM-clause entry`. См. VisitNegateExpression.
     */
    it('унарный минус ставит скобки вокруг операнда', () => {
      const { sql } = processQuery('$filter=-age lt -40');

      expect(sql).toContain('WHERE -(u.age) < :p0');
    });

    it('унарный минус перед выражением сохраняет его группировку', () => {
      const { sql } = processQuery('$filter=-(age add 4) lt -45');

      expect(sql).toContain('WHERE -(((u.age + :p0))) < :p1');
    });

    /**
     * Отрицательный литерал — не унарный минус: знак входит в само значение и уезжает
     * в параметр, узла отрицания не возникает. Тест закрепляет именно это различие:
     * иначе первый же рефакторинг парсера мог бы свести их к одному узлу и вернуть
     * знак в текст SQL.
     */
    it('отрицательный литерал уходит в параметр, а не в текст SQL', () => {
      const { sql, parameters } = processQuery('$filter=age gt -1');

      expect(sql).toContain('WHERE u.age > :p0');
      expect(parameters.get('p0')).toBe(-1);
    });
  });
